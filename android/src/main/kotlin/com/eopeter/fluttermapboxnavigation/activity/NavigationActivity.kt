...
class NavigationActivity : AppCompatActivity() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        FlutterMapboxNavigationPlugin.eventSink?.success(
            mapOf("event" to MapBoxEvents.NAVIGATION_CANCELLED)
        )
        isNavigationInProgress = false
        // Não chame finish() aqui para evitar fechar o app
    }
}
...
override fun onDestroy() {
    ...
    // Remove NavigationView listeners
    if (FlutterMapboxNavigationPlugin.longPressDestinationEnabled) {
    ...
    // Unregister all MapboxNavigation observers
    ...
    // **Desregistrar os BroadcastReceivers**
    finishBroadcastReceiver?.let {
        unregisterReceiver(it)
        finishBroadcastReceiver = null
    }
    addWayPointsBroadcastReceiver?.let {
        unregisterReceiver(it)
